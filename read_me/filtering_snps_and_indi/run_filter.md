...
We moved forward to the tab named globalfilt_params dedicated to site- and sample-based filtering (***/parameters/process/globalfilt_params.config**), where we specify parameters described at the begining of this read.me. At the end, save the parameters as yml file.
...
For generating the geographic map of samples (withut re-running the workflow), one can either use **geo_map_file.txt** generated by the workflow or preprepare a tab-delimited file in the similar format. Python script is runned like this: 
```
