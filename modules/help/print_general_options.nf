def print_help() {
    ...
    --f_chrom_len [file]         If the inputs are plink binary files, map file is needed to set the chromosome id and its respective size in the vcf header (for example, see: examples/chrm_size.map).
    ...
    For detailed options for each analysis:
        ...
        3). To see the options related to treemix, type "nextflow run scalepopgen.nf --help --treemix"
