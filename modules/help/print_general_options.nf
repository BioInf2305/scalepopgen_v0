def print_help() {
    log.info"""
    Usage: nextflow run scalepopgen.nf --input ../example_plink/*.{bed,bim,fam} --outDir ../example_out/ 

    Required arguments:

    --input [file]           In case of vcf, the input should be ".csv" with first column as chromosome id, second column as path to the vcf file and                                                           third column, path to its respective index (for example, see: example/input_vcf.csv). In case of plink,  the input                                                                 should be directly the path to the bed file with its extension, "*.{bim,bed,fam}". Note that there should ONLY be ONE SET                                                          OF PLINK binary files in specified path. 

    --sample_map [file]      Path to the sample map file (for example, see example/sample_pop.map), format: first column, sample id and the second column, population id. This
                             is REQUIRED arguments only if the input is ".csv"

    --outDir [dir]           Path to the directory, where all the outputs will be stored. If the directory is not present, it will be created. 

    Optional arguments:
                           
    --geo_plot_yml [file]     Path to the yaml file containing parameters for plotting the samples on map (for example, see example/plot_sample_on_map.yaml). Refer to                                           doc/plot_map.md for description of this yaml file

    --tile_yml [file]         Path to the yaml file containing parameters for geographical map to be used for plotting (for example, see example/tiles_info.yaml). Refer to                                      doc/plot_map.md for description of this yaml file
    ...
    --chrm_map [file]         If the inputs are plink binary files, map file is needed to set the chromosome id and its respective size in the vcf header.                                                       (for example, see plink_example/chrm_size.map). If not provided, the greatest coordinate for each chromosome will be considered
                              its total size. 
